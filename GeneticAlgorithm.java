...
public class GeneticAlgorithm {
	...
	Population crossoverPopulation(Population population) {
		...
		IntStream.range(Driver.NUM_OF_ELITE_SCHEDULES, population.getSchedules().size()).forEach(x -> {
			if (Driver.CROSSOVER_RATE > Math.random()) {
				Schedule schedule1 = selectTournamentPopulation(population).sortSchedulesByFitness().getSchedules().get(0);
				Schedule schedule2 = selectTournamentPopulation(population).sortSchedulesByFitness().getSchedules().get(0);
	...
	Schedule mutateSchedule(Schedule schedule) {
		Schedule mutateSchedule = new Schedule(data).initialize();
	...
	Population selectTournamentPopulation(Population population) {
		...
		IntStream.range(0, Driver.TOURNAMENT_SELECTION_SIZE).forEach(x -> {
			...
			tournamentPopulation.getSchedules().set(x, population.getSchedules().get(randomIndex));
		});
